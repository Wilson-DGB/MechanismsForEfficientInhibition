...
NPD1 = 100 # number of PD1 receptors
...
for z = 1:length(PDL1Span)
    ...
    k5_star = 100.0 # ligand binding rate
    km5 = 100.0 # ligand unbinding rate

    k4_star = 10.0 # strength of the PD1 receptor inhibition

        # saving parameters
        dosave      = true
        outfname    = "/home/daniel/Documents/MechanismsForEfficientInhibition/ModellingLigandInteractions/LigandLocalisation/WellMixedLigandData/WellMixedLigandModel-CaseNeither-k4=$(k4_star)-k5=$(k5_star)-km5=$(km5)-CD80Number=$(NCD80)-PDL1Number=$(PDL1Span[z])-MolecularReach=$(MR)-N=$(N)-PD1=$(NPD1)-CD28=$(NCD28)-tf=$(tf)-dt=$(dt)-nsims=$(Nsim).csv"
        ...
        function RunSolver(k4_star,km5,Nsim,tf,dt,NA,NB,NC,ND,NE,NCD80,N,wrappeddist,k5_star)
            
            NS = ceil(Int,tf/dt)+1

            CCopyNumber = zeros(NS,Nsim)
            ZCopyNumber = zeros(NS,Nsim)
            Tspan = 0:dt:(NS-1)*dt
            ...
            RateMatrix = readdlm("PretabulatedRates/CRDME_Rates_nanom_31by31Grid_L=300_N=64_eps=15.txt")
            ...
            for nsim=1:Nsim
                ...
                Propensities[4:3+NT] = k1*current_CD80*(ParticleType .== 1) + (k2+km1)*(ParticleType .== 2) + (km2+km1)*(ParticleType .== 3) + k3*current_PDL1*(ParticleType .== 4) + km3*(ParticleType .== 5)
                ...
                while t < tf
                    ...
                    while t>Tspan[count] && count<NS
                        # we save the total number of C and Z molecules
                        ZCopyNumber[count,nsim] = current_CD80_PDL1
                        ...
                        elseif ReactType[RX] == 2
                            ...
                            for k=1:NPD1
                                if ParticleType[k+NCD28]==4
                                    ...
                                    Propensities[3+NCD28+k] = k3 * current_PDL1
                                    a0 += Propensities[3+NCD28+k]
                                end
                            end
                ...
                ZCopyNumber[NS,nsim] = current_CD80_PDL1
            ...
            return CCopyNumber, ZCopyNumber
        end


        @time CCopyNumber, ZCopyNumber = RunSolver(k4_star,km5,Nsim,tf,dt,NA,NB,NC,ND,NE,NCD80,N,wrappeddist,k5_star)

        # Save the data into a csv file
        if dosave
            df = DataFrame()
            df[!,Symbol("C Count")] = CCopyNumber[:,1]
            df[!,Symbol("Z Count")] = ZCopyNumber[:,1]
