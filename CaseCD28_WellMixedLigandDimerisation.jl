...
NPD1 = 10 # number of PD1 receptors
...
#I0Span = [1000000];
TSpan = [1000.0,1000.0,1000.0,1000.0,1000.0,1000.0,1000.0,1000.0,100.0,100.0]*10
DTSpan = [1e-2,1e-2,1e-2,1e-2,1e-2,1e-2,1e-2,1e-2,1e-3,1e-3]*10
...
k5_star = 100.0 # phosphorylation rate
    ...
    # saving parameters
    dosave      = true
    outfname    = "/home/daniel/Documents/MechanismsForEfficientInhibition/ModellingLigandInteractions/DoseResponseCurves/AllPathways/Ligand_Dimerisation/data/WellMixedLigandModel-CaseCD28-k5=$(k5_star)-CD80Number=$(NCD80)-PDL1Number=$(PDL1Span[z])-MolecularReach=$(MR)-N=$(N)-PD1=$(NPD1)-CD28=$(NCD28)-tf=$(tf)-dt=$(dt)-nsims=$(Nsim).csv"
    ...
    function RunSolver(Nsim,tf,dt,NA,NB,NC,ND,NE,NCD80,N,wrappeddist,k5_star)
        ...
        for nsim=1:Nsim
            ...
            Propensities[4:3+NT] = k1*(current_CD80+current_CD80_PDL1)*(ParticleType .== 1) + (k2+km1)*(ParticleType .== 2) + (km2+km1)*(ParticleType .== 3) + k3*current_PDL1*(ParticleType .== 4) + km3*(ParticleType .== 5)
                ...
                if t < tf # no need to simulate the reaction if it occurs after tf
                    ...
                    elseif ReactType[RX] == 2
                        ...
                        for k=1:NCD28
                            if ParticleType[k]==1
                                a0 -= Propensities[k+3]
                                Propensities[k+3] = k1*(current_CD80+current_CD80_PDL1)
                                a0 += Propensities[k+3]
                            end
                        end
                        for k=1:NPD1
                    ...
                    elseif ReactType[RX] == 4       
                        ...
                        if ParticleType[RP]==1
                            ...
                            ParticleType[RP]=2
                            # is the ligand a CD80 or CD80:PDL1 complex
                            if rand() < current_CD80/(current_CD80+current_CD80_PDL1)
                                # then the ligand was a CD80
                                current_CD80-=1
                                BoundLigandType[RP]=1
                            else
                                # then the ligand was a CD80
                                current_CD80_PDL1-=1
            ...
            #C = findall(x->x==3,ParticleType)
            #B = findall(x->x==2,ParticleType)
            #E = findall(x->x==5,ParticleType)
            #PDL1_Bound = findall(x->x==3,BoundLigandType)
            #NC = length(C)
            #NB = length(findall(x->x==2, ParticleType))
            #NE = length(findall(x->x==5, ParticleType))
            #ZZ=intersect(C,PDL1_Bound)
            #WW=intersect(B,PDL1_Bound)
            #NPDL1_C = length(ZZ)
            #NPDL1_B = length(WW)
            #@show current_CD80+current_CD80_PDL1+NB+NC, current_PDL1+current_CD80_PDL1+NE+NPDL1_B+NPDL1_C
