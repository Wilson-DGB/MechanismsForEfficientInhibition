...
k4 = 0.1 / (6.023 * 1e-7) # from Sci Adv paper translated to per molecule 6.03 1e-4 / 6.023 1e-7 is approx. 1000: Or Ying's paper 0.1 / 6.023 * 1e-7
k5 = 1000.0 # CD80 ligand binding rate to PDL1:      X+Y-->Z
km5 = 10.0 # unbinding rate of the CD80:PDL1 ligand complex  Z-->X+Y
Param = [Drecept,Dligand,k1,km1,k2,km2,k3,km3,k4,k5,km5]

NCD80 = 10 # number of CD80 ligand molecules. 
NPDL1 = 10 # number of PDL1 ligand molecules.
...
function Calculate_Propensities(Param,PosL,PosR,ReceptorType,LigandType,RV,LV,h,NhbdAX,NhbdDY,NhbdCE,NhbdXY,NStim,NInhib,NStimL,NInhibL,DoiRateMatrix,DoiRateGridCentre)
...
function Implement_Reaction!(RX,Param,PosR,PosL,NR,NL,NStim,NInhib,NStimL,NInhibL,rho,N,h,ReceptorType,LigandType,BoundLigand,BoundLigand2Ligand,NhbdAX,NhbdDY,NhbdCE,NhbdXY,a0,Propensities,Doicutoffd,DoiRateGridCentre,DoiRateMatrix,Gausscutoffd,GaussRateGridCentre,GaussRateMatrix,PlacementRates)
...
function Create_Nhbd_Matrix(PosL,PosR,NStim,NInhib,NStimL,NInhibL,Doicutoffd,Gausscutoffd)
...
function RunSolver(Nsim,Param,RIC,LIC,N,h,visualise)

    # Loads Rate Matrices for Bimolecular reactions
    DoiRateMatrix = readdlm("PretabulatedRates/CRDME_DoiRates_5by5_N64.txt")
    ...
    GaussRateMatrix = readdlm("PretabulatedRates/CRDME_Rates_nanom_31by31Grid_L=300_N=64_eps=15.txt")
    ...
    Data = readdlm("PretabulatedRates/PlacementRates_5by5_N64.txt")
    M = size(Data)[1]
    ...
    for nr=1:Nsim
        ...
        t=0
        count=1
        NhbdAX, NhbdDY, NhbdCE, NhbdXY = Create_Nhbd_Matrix(PosL,PosR,NStim,NInhib,NStimL,NInhibL,Doicutoffd,Gausscutoffd)
        Propensities,a0 = Calculate_Propensities(Param,PosL,PosR,ReceptorType,LigandType,RIC,LIC,h,NhbdAX,NhbdDY,NhbdCE,NhbdXY,NStim,NInhib,NStimL,NInhibL,DoiRateMatrix,DoiRateGridCentre)
        ...
        while t < tf
            ...
            a0 = Implement_Reaction!(RX,Param,PosR,PosL,NR,NL,NStim,NInhib,NStimL,NInhibL,rho,N,h,ReceptorType,LigandType,BoundLigand,BoundLigand2Ligand,NhbdAX,NhbdDY,NhbdCE,NhbdXY,a0,Propensities,Doicutoffd,DoiRateGridCentre,DoiRateMatrix,Gausscutoffd,GaussRateGridCentre,GaussRateMatrix,PlacementRates)
            ...
            while t>tSave[count]
                ...
                if visualise==true
                    L1 = findall(x->x==1,LigandType)
                    L2 = findall(x->x==2,LigandType)
                    L3 = findall(x->x==3,LigandType)
                    R1 = findall(x->x==1,ReceptorType)
                    R2 = findall(x->x==2,ReceptorType)
                    R3 = findall(x->x==3,ReceptorType)
                    R4 = findall(x->x==4,ReceptorType)
                    R5 = findall(x->x==5,ReceptorType)
    
                    X = [PosL[L1,1],PosL[L2,1],PosL[L3,1],PosR[R1,1],PosR[R2,1],PosR[R3,1],PosR[R4,1],PosR[R5,1]]
                    Y = [PosL[L1,2],PosL[L2,2],PosL[L3,2],PosR[R1,2],PosR[R2,2],PosR[R3,2],PosR[R4,2],PosR[R5,2]]
                    display(plot(X,Y,seriestype=:scatter,label=["X" "Y" "Z" "A" "B" "C" "D" "E"],  xlims=(0,N),ylims=(0,N)))
...
@time CCount = RunSolver(1,Param,RIC,LIC,N,h,visualise)
